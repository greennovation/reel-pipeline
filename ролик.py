...
def собрать_план(описание: dict[str, Any], слова: list[dict[str, Any]], стиль: dict[str, Any]) -> dict[str, Any]:
    """Собирает JSON-совместимый со ``stitch_reel.py`` план нарезки.

    Стиль пока не меняет формат плана. Он уже передаётся сюда, чтобы следующее
    расширение движка могло использовать тот же разобранный источник правды.
    """
    del стиль
    _проверить_описание(описание)
